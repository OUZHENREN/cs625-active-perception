#!/usr/bin/env python3
"""Request one P7 DetachableJoint transition and atomically capture its receipt."""
...
def main() -> None:
    ...
    rclpy.init()
    node = Node("p7_execute_attachment_capture")
    ...
    node.create_subscription(String, "/p7/attachment_status", on_status, 10)
    ...
    if publisher.get_subscription_count() < 1:
        raise RuntimeError("P7 attachment adapter subscription unavailable")

    command = {"command_id": arguments.command_id, "action": arguments.action}
    outgoing = String()
    ...
    while receipt is None and time.monotonic() < deadline:
        rclpy.spin_once(node, timeout_sec=0.1)
    if receipt is None:
        raise RuntimeError("P7 attachment receipt timeout")

    record = {
        "capture_schema": "p7_attachment_receipt_v1",
